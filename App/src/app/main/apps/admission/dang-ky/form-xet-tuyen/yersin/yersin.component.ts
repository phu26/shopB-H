...
import { exhaustMap, tap, takeWhile, scan, startWith, switchMap, filter, debounceTime } from 'rxjs/operators';


@Component({
  selector: 'app-form-xet-tuyen-hoc-ba-admin',
  templateUrl: 'yersin.component.html',
...
export class YersinComponent extends BaseFormXetTuyenComponent {
  ...
  loadNganhDaoTaos() {
    this._nganhDaoTaoService.allAsync()
      .then(nganhDaoTaos => {
        console.log('in')
        console.log('nganhdaotao 313', nganhDaoTaos);
  ...
  onChangeChuyenNganh(i, chuyenNganh: NganhDaoTao, nganhDaoTao: NganhDaoTao) {
